      ...
      <main className="max-w-7xl mx-auto py-8 px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-revolt-card border border-revolt-border rounded-2xl overflow-hidden">
            <div className="bg-revolt-darker px-6 py-4 border-b border-revolt-border">
              <h2 className="text-lg font-bold text-white flex items-center gap-3">
                <div className="w-8 h-8 bg-revolt-red/20 rounded-lg flex items-center justify-center">
                  <svg className="w-4 h-4 text-revolt-red" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                </div>
                Rol Paneli
              </h2>
              <p className="text-sm text-revolt-text-muted mt-1">Kullanıcıların seçebileceği rolleri belirleyin ve paneli gönderin.</p>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="bg-revolt-dark border border-revolt-border rounded-lg p-3 max-h-64 overflow-y-auto space-y-1">
                  {roles.length === 0 ? (
          ...
          <div className="bg-revolt-card border border-revolt-border rounded-2xl overflow-hidden">
            <div className="bg-revolt-darker px-6 py-4 border-b border-revolt-border">
              <h2 className="text-lg font-bold text-white flex items-center gap-3">
                <div className="w-8 h-8 bg-revolt-yellow/20 rounded-lg flex items-center justify-center">
                  <svg className="w-4 h-4 text-revolt-yellow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                  </svg>
                </div>
                Faceit Paneli
              </h2>
              <p className="text-sm text-revolt-text-muted mt-1">Her seviye için atanacak rolleri seçin ve paneli gönderin.</p>
            </div>
        ...
        {/* Mevcut Yapılandırma */}
        <div className="mt-8 bg-revolt-card border border-revolt-border rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-revolt-dark rounded-xl p-4 border border-revolt-border">
              <div className="text-2xl font-extrabold text-revolt-accent mb-1">
                {config?.configurableRoleIds?.length || 0}
              </div>
              <div className="text-sm text-revolt-text-muted">Yapılandırılmış Rol</div>
              {config?.rolePanelChannelId && (
                <div className="text-xs text-revolt-text-muted mt-2">
                  Kanal: {channels.find(c => c.id === config.rolePanelChannelId)?.name || config.rolePanelChannelId}
                </div>
              )}
            </div>

            <div className="bg-revolt-dark rounded-xl p-4 border border-revolt-border">
              <div className="text-2xl font-extrabold text-revolt-yellow mb-1">
                {Object.keys(config?.faceitLevelRoles || {}).length}
              </div>
              <div className="text-sm text-revolt-text-muted">Faceit Seviye Rolü</div>
              {config?.faceitPanelChannelId && (
                <div className="text-xs text-revolt-text-muted mt-2">
                  Kanal: {channels.find(c => c.id === config.faceitPanelChannelId)?.name || config.faceitPanelChannelId}
                </div>
              )}
            </div>

            <div className="bg-revolt-dark rounded-xl p-4 border border-revolt-border">
              <div className="text-2xl font-extrabold text-revolt-green mb-1">24/7</div>
              <div className="text-sm text-revolt-text-muted">Çalışma Süresi</div>
            </div>
          </div>
        </div>
      </main>
